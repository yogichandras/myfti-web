...
<?php if (count($show_in_filter) > 0): ?>
          <a href="{php_open_tag_echo}url("<?=strtolower($controller)?>/filter/"){php_close_tag}" id="filter-show" class="btn btn-sm btn-success btn-icon-text"><i class="mdi mdi-magnify btn-icon-prepend"></i> Filter</a>
<?php endif; ?>
        </div>
...
<?php endif; ?>
  ...
  $(document).on("click","#delete",function(e){
    ...
    $('#modalContent').html(`<p class="mb-4">{php_open_tag_echo}cclang("delete_description"){php_close_tag}</p>
  	                          ...
  	                          <button type='button' class='btn btn-success btn-sm' id='ya-hapus' data-id=`+$(this).attr('alt')+`  data-url=`+$(this).attr('href')+`>{php_open_tag_echo}cclang("delete_action"){php_close_tag}</button>
  														`);
    $("#modalGue").modal('show');
  });
